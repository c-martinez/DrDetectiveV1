$(document).ready(function () {  
	...
	button.innerHTML = "Show clues by others";
...
function submitAnswers() {
	...
	var Item = makeStruct("timeStart timeCreate factorPos factorExpl factorTimes otherButtonToggle");
	timeCreate = new Date();
	
	var bt = otherButtonToggle;
	//alert(otherButtonToggle);
	//alert(bt);
...
function nextPatient(url) {
	...
	$("#scoreForm").attr("value", currScore + consScore * docScore);
...
//	alert(submitted);
	...
	for (var i = 0; i < allAnn.length; i++) {
		...
		text = document.createElement("span");
		text.setAttribute("class", "popular-tag");
