...
/*function parse_existing_factors($pid, $con) {
	$query = "SELECT ID, Worker_ID
		FROM game_simple_output_saved_factors
	...
	while ($row = mysqli_fetch_array($res)) {
		...
		$query = "UPDATE game_simple_output_saved_factors
...
function update_factor_status($wid, $pid, $factor, $con) {
	$query = "DELETE FROM game_simple_output_saved_factors
		WHERE Worker_ID = '$wid' AND
		Par_ID = $pid AND
		Factor = '$factor'";
	mysqli_query($con, $query);
...
function update_submitter_score($pid, $word, $term, $factor_type, $worker_role, $fac_or_rel, $con) {
	if (strcmp($fac_or_rel, "factors") == 0) {
		$query = "SELECT DISTINCT(Worker_ID), Time_create, Factors_list
			FROM game_simple_output_factors WHERE 
			Par_ID = $pid AND
		...
		$query = "SELECT DISTINCT(Worker_ID), Time_start
			FROM game_simple_output_relations WHERE 
			Par_ID = $pid AND
			Worker_role = '$worker_role' AND
...
if (isset($_POST['time_start'])) {
	...
	if (strcmp($_POST['fac_rel'], "factors") == 0 ) {
		...
		foreach ($factor_classes as $fc) {
			foreach ($available_factor_types[$fc] as $ft) {
				if (isset($results[$ft])) {
					...
					$query = "INSERT INTO game_simple_output_factors
						(Par_ID, Job_ID, Worker_ID, Worker_role, Factor_class, Factor_type, Factors_count, Factors_list, Time_start, Time_create, Relation_role)
					...
					$query = "UPDATE game_user SET Score = Score + $score WHERE
						Name = '$wid'";
					mysqli_query($con, $query);
		...
		if ($found_answers == false) {
					$query = "INSERT INTO
						game_simple_output_factors (Par_ID, Job_ID, Worker_ID, Worker_role,Time_start, Time_create, Relation_role, Factors_count)
...
if (strcmp($url,"submit") == 0) {
	...
	$query = "UPDATE game_simple_output_$fac_or_rel
