...
$query = "SELECT
		...
		WHERE NOT EXISTS (
		SELECT * FROM game_simple_output_factors WHERE
  	game_simple_output_factors.Par_ID = game_doc_par.ID AND
  	game_simple_output_factors.Worker_ID = '$wid' AND
  	game_simple_output_factors.Worker_role = '$worker_role' AND
  	game_simple_output_factors.Factors_count > -2) AND
...
$query = "SELECT Factor_class, Factor_type, Factors_count, Factors_list, Relation_role FROM game_simple_output_factors WHERE
	Par_ID = $pid AND
...
if (strcmp($worker_role, "validate") == 0) {
	$query = "SELECT Factor_class, Factor_type, Factors_count, Factors_list FROM game_simple_output_factors WHERE
		Par_ID = $pid AND
