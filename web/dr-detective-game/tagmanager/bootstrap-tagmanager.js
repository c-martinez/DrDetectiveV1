...
(function($){
  ...
  $.fn.tagsManager = function (options,tagToManipulate) {
    ...
    var spliceTag = function (tagId) {
      ...
      if (-1 != p) {
//        alert($("#" + objName + "_" + tagId).text());
        
        var str = $("#" + objName + "_" + tagId).text();
        str = str.substring(0, str.length - 1);
        //alert(str);
        
        if (fr == "relations") {
		      if (workerRole == "annotate") {
			      var i = $.inArray(str, relations);
		      	if (i > -1) {
		      		$("#"+relations[i]).css("text-decoration", "none");
		      		factorTimes.splice(i,1);
		      		relations.splice(i,1);
		      	}
		      } else {
		      	if ($.inArray(str, relations) > -1) {
		      		i = $.inArray(str, relations);
		      		//alert(relations[i]);
		      		jQuery("#validateTags").tagsManager('pushTag',relations[i]);
		      		removedRelations.push(relations[i]);
		      		var timeCreate = new Date();
		      		factorTimes.push(timeCreate.getTime());
		      		relations.splice(i,1);
		      	}
		      	else if ($.inArray(str, removedRelations) > -1) {
		      		i = $.inArray(str, removedRelations);
		      		//alert(removedRelations[i]);
		      		jQuery("#answerTags").tagsManager('pushTag',removedRelations[i]);
		      		relations.push(removedRelations[i]);
		      		removedRelations.splice(i,1);
		      		factorTimes.splice(i,1);
		      	}
		      }
        }
        else {
						var i = $.inArray(str, factorExpl);
		      	
		      	if (i > -1) {
		      		for (var j = 0; j < factors[i].length; j++) {
		      			if (factors[i][j].css("background-color") != colorMap["purple"] ) {
		      				factors[i][j].css("background-color", colorMap["white"]);
		      			} else {
		      				factors[i][j].css("background-color", colorMap["blue"]); // blue words (i.e. submitted by other users) 
		      			}
		      		}
							...
							if (workerRole == "annotate") {
					   		factors.splice(i,1);
						 		factorTimes.splice(i,1);
					   		factorExpl.splice(i,1);
			     		}
			     		else {
		      			jQuery("#validateTags").tagsManager('pushTag',factorExpl[i])
		      			
		      			removedFactors.push(factors[i]);
		      			var timeCreate = new Date();
		      			factorTimes.push(timeCreate.getTime());
		      			removedFactorsExpl.push(factorExpl[i]);
		      	...
		      	else if (workerRole == "validate") {
		      		var j = $.inArray(str, removedFactorsExpl);
		      		if (j > -1) {
		      			jQuery("#answerTags").tagsManager('pushTag',removedFactorsExpl[j]);
		      		
		      			factors.push(removedFactors[j]);
		      			factorExpl.push(removedFactorsExpl[j]);
		      			
		      			removedFactors.splice(j,1);
		      			removedFactorsExpl.splice(j,1);
		      			factorTimes.splice(j,1);
		      			colorText();
		      		}
		      	}
        }
        ...
        refreshHiddenTagList();
        // console.log(tlis);
        //alert("Factors:" + factors.length + ", RemFac:" + removedFactors.length);
      }
