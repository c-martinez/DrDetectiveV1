...
function solved_score($par_id, $fac_or_rel, $worker_role, $con) {
	$query = "SELECT COUNT(ID) FROM game_output_$fac_or_rel WHERE
...
function count_paragraphs_per_domain($uid, $domains, $level, $con, $exp_group) {
	...
	while ($row = mysqli_fetch_array($res)) {
		$pid = $row["ID"];
		$diff = par_diff_score($pid, $uid, $con);
...
function count_solved_paragraphs_per_domain($uid, $domains, $level, $con, $exp_group) {
		$condition = "";
		if ($exp_group == 1) {
			$condition = "AND game_doc_par.ID < 11";
		}
	...
	$query = "SELECT DISTINCT(game_doc_par.ID) FROM game_doc_par
		INNER JOIN game_doc ON game_doc_par.Doc_ID = game_doc.ID
		INNER JOIN game_output_factors ON game_doc_par.ID = game_output_factors.Par_ID
		WHERE game_doc.Domains LIKE '%$domains%' AND 
  	game_output_factors.Worker_ID = '$uid' AND
  	game_output_factors.Factors_count > -2
  	$condition";
	//echo $query;
	$res = mysqli_query($con, $query);
